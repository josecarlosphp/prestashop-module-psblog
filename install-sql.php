...
Db::getInstance()->Execute(
	'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'blog_category_lang` (
...
Db::getInstance()->Execute(
	'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'blog_post_lang` (
...
Db::getInstance()->Execute(
	'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'blog_category_shop` (
        `id_blog_category` int(10) unsigned NOT NULL,
...
Db::getInstance()->Execute(
	'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'blog_comment` (
...
Db::getInstance()->Execute(
	'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'blog_image` (
...
Db::getInstance()->Execute(
	'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'blog_post_relation` (
...
Db::getInstance()->Execute(
	'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'blog_category_relation` (
...
Db::getInstance()->Execute(
	'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'blog_post_shop` (
        `id_blog_post` int(10) unsigned NOT NULL,
        `id_shop` int(10) unsigned NOT NULL,
...
Db::getInstance()->Execute(
	'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'blog_visit` (
...
$categories = array();
$categories[] = array(
	'name' => array('en' => 'Blog', 'fr' => 'Blog', 'es' => 'Blog'),
	'link' => array('en' => 'blog', 'fr' => 'blog', 'es' => 'blog')
);

$categories[] = array(
	'name' => array('en' => 'Blog featured posts', 'fr' => 'Articles à la une', 'es' => 'Artículos destacados'),
...
foreach ($categories as $value) {
	...
	Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'blog_category` WHERE `id_blog_category` = '.(int)$i);
	Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'blog_category`(`id_blog_category`,`active`,`position`,`id_blog_category_parent`) VALUES ('.(int)$i.', 1, 0, 0)');
	Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'blog_category_lang` WHERE `id_blog_category` = '.(int)$i);
	...
	foreach ($languages as $l) {
		...
		$title = array_key_exists(
			$l['iso_code'],
			$value['name']
		) ? $value['name'][$l['iso_code']] : $value['name']['en'];
		$link = array_key_exists(
			$l['iso_code'],
			$value['link']
		) ? $value['link'][$l['iso_code']] : $value['link']['en'];

		Db::getInstance()->Execute(
			'INSERT INTO `'._DB_PREFIX_.'blog_category_lang`(`id_blog_category`,`id_lang`,`name`,`description`,`link_rewrite`,`meta_keywords`,`meta_description`)
										VALUES('.(int)$i.','.(int)$l['id_lang'].',"'.$title.'","'.$title.'", "'.$link.'", "'.$title.'","'.$title.'")'
		);
	...
	Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'blog_category_shop` WHERE `id_blog_category` = '.(int)$i);
	foreach ($shops as $shop) {
		Db::getInstance()->Execute(
			'INSERT INTO `'._DB_PREFIX_.'blog_category_shop`(`id_shop`,`id_blog_category`) VALUES('.(int)$shop['id_shop'].','.(int)$i.')'
		);
	}
