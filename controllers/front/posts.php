...
class PsblogPostsModuleFrontController extends ModuleFrontController
{
	...
	public function init()
	{
		...
		$this->list_link = BlogLink::linkList();
	}
	...
	public function displayList()
	{
		...
		$limit_per_page = (int)$this->conf['list_limit_page'];
		$current_page = Tools::getIsset('p') && is_numeric(Tools::getValue('p')) ? (int)Tools::getValue('p') : 1;
				...
				if (!is_null($category->id_blog_category_parent) && !empty($category->id_blog_category_parent)) {
					...
					if (!$parent_category->isAllowed()) {
						...
						$this->context->smarty->assign(
							...
							BlogLink::linkCategory($parent_category->id, $parent_category->link_rewrite)
						);
					}
				}
			}
		...
		if ($category && $category->active && $category->id != 1) {
			...
			$list = $category->getPosts(true, true, $start, (int)$this->conf['list_limit_page']);
			...
			$categoryLink = BlogLink::linkCategory($category->id, $category->link_rewrite);
			$paginationLink = BlogLink::linkCategory($category->id, $category->link_rewrite, '');
			...
			if (Tools::getIsset('search') && Tools::getValue('search') != '') {
				...
				$this->context->smarty->assign('paginationLink', BlogLink::linkList('', null, array('search='.$search_query)));

			} elseif (!is_null($this->year) || !is_null($this->month)) {

				/** archives ***/
				$nb_articles = BlogPost::listPosts(
					true,
					true,
					null,
					null,
				...
				$this->context->smarty->assign('paginationLink', BlogLink::linkList('', null, $params));
				...
				$this->context->smarty->assign('paginationLink', BlogLink::linkList(''));
			}
		...
		$this->setTemplate('module:psblog/views/templates/front/list.tpl');
	}
	...
	public function displayPost()
	{
		...
		$postPublished = false;
		if ($post && $post->status == 'published' && strtotime($post->date_on) < time()) {
			$postPublished = true;
		}

		if ($post && $postPublished && $post->isAssociatedToLang($id_lang) && $post->isAssociatedToShop($id_shop)) {
			...
			if (Tools::isSubmit('submitMessage') && $this->conf['comment_active'] && $post->allow_comments) {
				...
				try {
					...
					if ($this->conf['antispam'] == 1) {
						$spamKey = Tools::getValue('blog_spam_key');
						$totalPost = Tools::getValue('blog_spam_answer');
						if (!$spamKey || !$totalPost) {
							Throw new Exception('antispam_invalid');
						}
						$operationArr = explode('_', $spamKey);
						$total = $operationArr[0] + $operationArr[1];
						if ($total != $totalPost) {
							Throw new Exception('antispam_invalid');
						}
					}
					...
					if ($message == '' || Tools::strlen($comment->customer_name) < (int)$this->conf['comment_name_min_length']) {
					...
					$comment->save();
					$comment->sendCommentNotification($post);
			...
			if ($this->conf['view_display_popin'] == 1) {
				...
				$this->addJS($this->module->getPathUri().'views/js/popin.js');
			}
			...
			if ($this->conf['antispam'] == 1) {
				...
				$key = $num1.'_'.$num2;
				$this->context->smarty->assign(
					array('blog_spam_operation' => $num1.' + '.$num2,
						  'blog_spam_key' => $key)
				);
			}
		...
		$this->setTemplate('module:psblog/views/templates/front/view.tpl');
	}
	...
	public function initContent()
	{
		...
		$this->context->smarty->assign(
			array(
				'posts_img_path' => _PS_BASE_URL_SSL_.__PS_BASE_URI__.$img_path,
				'blog_conf' => $this->conf,
