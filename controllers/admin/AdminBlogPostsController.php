 ...
 *
 * AdminBlogPostsController tab for admin panel
 */
...
class AdminBlogPostsController extends ModuleAdminController
{
	...
	public function __construct()
	{
		...
		$icon_array = array(
			'published' => array('class' => 'icon-check text-success', 'alt' => $this->l('Published')),
			'suspended' => array('class' => 'icon-remove text-danger', 'alt' => $this->l('Suspended')),
			'drafted' => array('class' => 'icon-pencil text-warning', 'alt' => $this->l('Drafted')),
		);
		...
		$this->fields_list = array(
			...
			'title' => array('title' => $this->l('Title'), 'width' => 400, 'filter_key' => 'b!title')
		);
		...
		$this->fields_list['date_on'] = array(
	...
	public function setMedia($isNewTheme = false)
	{
		...
		$this->addJS(
			array(
				_MODULE_DIR_.$this->module->name.'/views/js/psblog.js',
				_MODULE_DIR_.$this->module->name.'/views/js/jquery.MultiFile.pack.js'
			)
		);
	...
	public function renderList()
	{
		...
		$l = array(
			'comments_waiting' => $this->l('Comments awaiting moderation'),
	...
	public function renderForm()
	{
		...
		$this->fields_value['groupBox'] = Tools::getIsset('groupBox') ? Tools::getValue(
			'groupBox'
		) : $obj->listCategories($defaultLanguage, false, false, true);

		$articles = BlogPost::listPosts(false, false, null, null, false, null, null, $obj->id);
		$this->fields_value['groupRelated'] = Tools::getIsset('groupRelated') ? Tools::getValue(
			'groupRelated'
		) : $obj->getRelatedPosts(false, false, true);
		...
		if (isset($this->conf['comment_active']) && $this->conf['comment_active'] == 1) {
			...
			$this->fields_value['allow_comments'] = (!$this->object->id) ? 1 : $this->getFieldValue(
				$this->object,
				'allow_comments'
			);
		...
		if (Shop::isFeatureActive()) {
			if (Shop::getTotalShops(false) == 1) {
				...
				$shop = Shop::getShops(false, null, true);
				$shop_val = array_values($shop);
				if (isset($shop_val[0])) {

					$name = 'checkBoxShopAsso_blog_post['.$shop_val[0].']';
					$this->fields_form[4]['form']['input'][] = array(
						'type' => 'hidden',
						'name' => 'checkBoxShopAsso_blog_post['.$shop_val[0].']'
					);
					$this->fields_value[$name] = $shop_val[0];
				}
		...
		$this->tpl_form_vars['firstCall'] = false; //hack for ps translate bug
		$this->tpl_form_vars['ajax_product_link'] = $this->context->link->getAdminLink('AdminBlog').'&ajax=1&action=getproductlinks';
				...
				if (isset($_FILES['blog_img']) && !empty($_FILES['blog_img']['tmp_name'][0])) {
					...
					foreach ($_FILES['blog_img']['name'] as $key => $val) {
						...
						try {
							...
							//img name
							$curr_time = time() + $j;
							$ext = Tools::substr(strrchr($file['name'], '.'), 1);
	...
	protected function checkFile($_file, $save_path)
	{
		...
		$ext = Tools::substr(strrchr($file_name, '.'), 1);
		if (count($supported_formats) > 0) {
			if (!in_array($ext, $supported_formats)) {
				throw new Exception("File format \".".$ext."\" is not allowed  (".implode(
						',',
						$supported_formats
					).")");
			}
		...
		//check filename
		if (Tools::strlen($file_name) == 0 || Tools::strlen($file_name) > $MAX_FILENAME_LENGTH) {
			...
			foreach ($this->fields_list as $key => $params) {
				...
				if ($key == 'image') {
					if ($params['image'] != 'p' || Configuration::get('PS_LEGACY_IMAGES')) {
						$path_to_image = Tools::getShopDomain(
								true
							)._PS_IMG_.$params['image'].'/'.$row['id_'.$this->table].(isset($row['id_image']) ? '-'.(int)$row['id_image'] : '').'.'.$this->imageType;
					} else {
						$path_to_image = Tools::getShopDomain(
								true
							)._PS_IMG_.$params['image'].'/'.Image::getImgFolderStatic(
								$row['id_image']
