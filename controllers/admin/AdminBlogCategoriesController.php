 ...
 *
 * AdminBlogCategoriesController tab for admin panel
 */
...
class AdminBlogCategoriesController extends ModuleAdminController
{
	public function __construct()
	{
		...
		$this->fields_list['parent_category'] = array('title' => $this->l('Parent'), 'width' => 240, 'search' => false);
	...
	public function setMedia($isNewTheme = false)
	{
		...
		$this->addJS(array(_MODULE_DIR_.$this->module->name.'/views/js/psblog.js'));
	}
	...
	public function renderForm()
	{
		...
		foreach ($customer_groups as $group) {
			$this->fields_value['groupBox_'.$group['id_group']] = Tools::getValue(
				...
				in_array($group['id_group'], $category_groups)
			);
		}
		...
		if (Shop::isFeatureActive()) {
			if (Shop::getTotalShops(false) == 1) {
				...
				if (isset($shop_val[0])) {
					...
					$name = 'checkBoxShopAsso_blog_category['.$shop_val[0].']';
					$this->fields_form['input'][] = array(
						'type' => 'hidden',
						'name' => 'checkBoxShopAsso_blog_category['.$shop_val[0].']'
					);
