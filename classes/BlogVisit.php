...
class BlogVisit extends ObjectModel
{
	...
	public static function setNewConnexion($type_page, $id_page)
	{
		...
		$query = 'SELECT * FROM `'._DB_PREFIX_.'blog_visit` v
                  ...
                  AND v.`id_page` = '.$id_page.' AND v.`ip_address` = "'.(int)$ip_adress.'"';

		$result = Db::getInstance()->getRow($query);

		if ($result) {
			$id_blog_visit = $result['id_blog_visit'];
			Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'blog_visit` SET `counter` = `counter`+1 WHERE `id_blog_visit` = '.(int)$id_blog_visit);
		} else {

			$visit = new BlogVisit();
			$visit->id_page = (int)$id_page;
			$visit->type_page = pSQL($type_page);
			$visit->ip_address = (int)$ip_adress;
			$visit->counter = 1;
			$visit->add();
